...
from .curve import menger_curvature

__all__ = [
    "improve_race_line",
    "improve_race_line_runner",
]


def improve_race_line(old_line, inner_border, outer_border, xi_iterations=4):
...
def improve_race_line_runner(track_vars_shapely: dict, line_iterations: int, xi_iterations: int):
    print(len(track_vars_shapely['center_line']))
    # start along centerline of track
    race_line = copy.deepcopy(track_vars_shapely['center_line'][:-1])  # Use this for centerline being outer bound
    for i in range(line_iterations):
        race_line = improve_race_line(
            race_line,
            track_vars_shapely['inner_border'],
            track_vars_shapely['outer_border'],
            xi_iterations
        )  # Remove "_new" for entire track width
